...
const About: React.FC<AboutProps> = ({ useElementOnScreen }) => {
  const [bannerContactRef, bannerContacVisible] = useElementOnScreen({
    threshold: 0.3,
  });
  const [nuestraHistoriaRef, nuestraHistoriaVisible] = useElementOnScreen({
    threshold: 0.1,
  });
    ...
    <main className="w-full bg-background-white">
      ...
      <header className="relative bg-[#2D1B14] text-white">
        ...
        <div className="relative h-[28vh] md:h-[35vh] flex items-center justify-center overflow-hidden">
          ...
          >
            ...
            <p
              className={`font-opensans text-sm md:text-xl max-w-2xl mx-auto ${
                bannerContacVisible
      ...
      <section className="py-16 bg-white-1">
        <div className="container mx-auto px-4">
          <article
            ref={nuestraHistoriaRef}
            className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center"
          >
            <div>
              <h2
                className={`font-lato font-bold text-2xl text-primary mb-4 ${
                  nuestraHistoriaVisible
              ...
              <p
                className={`font-opensans text-sm md:text-base text-white-2 mb-4 ${
                  nuestraHistoriaVisible
                    ? "animate__animated animate__fadeInLeft"
                    : "opacity-0"
                }`}
              >
                Donde Carla nació hace más de 2 años como un pequeño negocio
                familiar en Juan Viñas. Fundado por Carla Morales, quien siempre
                soñó con compartir sus recetas familiares con la comunidad.
              </p>
              <p
                className={`font-opensans text-sm md:text-base text-white-2 mb-4 ${
                  nuestraHistoriaVisible
        ...
        <div className="container mx-auto px-4">
          <header
            className={`text-center mb-12 ${
              misionVisible ? "animate__animated animate__fadeIn" : "opacity-0"
            }`}
          >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <article
              ref={misionRef}
              className={`bg-white-1 p-6 rounded-lg shadow-md ${
                misionVisible
                  ? "animate__animated animate__fadeInLeft"
                  : "opacity-0"
              }`}
            >
              <h3 className="font-lato text-xl font-semibold text-primary mb-4">
                Misión
              </h3>
              ...
              <ul className="space-y-2">
                {values.map((value, index) => (
                  <li key={index} className="flex items-start">
                    ...
                    <span className="font-opensans text-sm md:text-base text-white-2">
                      {value}
                    </span>
                  </li>
                ))}
